 ...
 */
export default function (olLayer, glStyle, sources, resolutions = defaultResolutions, spriteData, spriteImageUrl, getFonts) {
        ...
        for (let i = 0, ii = layers.length; i < ii; ++i) {
            ...
            if (!filter || evaluateFilter(layerId, filter, f, zoom)) {
                ...
                if (label && opacity && !skipLabel) {
                    ...
                    let wrappedLabel = type == 2 ? applyLetterSpacing(label, letterSpacing) : wrapText(label, font, maxTextWidth, letterSpacing);
                    wrappedLabel = wrappedLabel.replace(/\|/g, "");
